...
**Done 2026-06-18:**
- **xdebug** (`rpmbuild-php-xdebug`): cap blocker. Updated `php-pecl-xdebug.spec` 3.5.0→3.5.3, cap `< 8.5`→`< 8.6`, switched `Source0` from git-commit snapshot to release-tag archive (`archive/<ver>/xdebug-<ver>.tar.gz`, extracts to `xdebug-<ver>/`), swapped `SOURCES/` tarball. Changelog: Remi's upstream `3.5.3-1` entry verbatim — cap bump matches rawhide so no own entry (see [[changelog-author-identity]]). **Gotcha:** a surgical version bump must also carry over the upstream `%check` changes — 3.5.3 added two `rm tests/...phpt` exclusions (bug02424, `map-minimum-path/minimum-path.phpt`) for env-dependent failures; missing them let `%check` run a test that fails in the build container. When bumping, diff the *whole* upstream spec (`%check` test-skips, `BuildRequires`), not just version/Source0/cap.
